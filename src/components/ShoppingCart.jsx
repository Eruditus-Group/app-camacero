  ...
  const handleCheckout = useCallback(async () => {
    if (cartItems.length === 0) {
      toast({
        ...
        description: 'Añade productos a tu carrito antes de pagar.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      const successUrl = `${window.location.origin}/exito`;
                ...
                cartItems.map(item => (
                  <div key={item.variant.id} className="flex items-center gap-4 bg-gray-800/50 p-3 rounded-lg">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      <Button onClick={() => removeFromCart(item.variant.id)} size="sm" variant="ghost" className="text-red-400 hover:text-red-300 text-xs">Eliminar</Button>
                    </div>
                  </div>
                ))
              )}
            ...
            {cartItems.length > 0 && (
              <div className="p-6 border-t border-white/10">
                <div className="flex justify-between items-center mb-4 text-white">
                  ...
                  <span className="text-2xl font-bold">{formatCurrency(getCartTotal(), cartItems[0].variant.currency)}</span>
                </div>
                <Button onClick={handleCheckout} className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-3 text-base">
                  Proceder al Pago
                </Button>
              </div>
            )}
